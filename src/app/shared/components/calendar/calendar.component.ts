...
export class SharedCalendarComponent {
  ...
  markers = input<Date[]>([]); // Dates that should have a dot
  ...
  days = computed(() => {
    ...
    // Days
    const markerDates = this.markers();
    ...
    for (let i = 1; i <= daysInMonth; i++) {
      ...
      const hasEntry = this.hasMarker(d, markerDates);
  ...
  private hasMarker(date: Date, markers: Date[]): boolean {
    return markers.some(m => this.isSameDay(new Date(m), date));
  }
